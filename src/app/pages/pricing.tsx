    ...
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <header className="border-b border-[#334155] bg-[#0F172A]/95 backdrop-blur supports-[backdrop-filter]:bg-[#0F172A]/60 sticky top-0 z-50">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link to="/pricing" className="text-sm text-[#00FF41]">Pricing</Link>
            {isAuthenticated ? (
              <Link to="/dashboard">
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="text-center mb-16">
          <Badge className="bg-[#1E293B] text-[#3B82F6] border-[#334155] font-mono mb-4">
            Pricing Plans
          </Badge>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Choose Your <span className="text-[#00FF41]">Learning Path</span>
          </h1>
        ...
        {isAuthenticated && user?.plan && (
          <div className="text-center mb-8">
            <Badge className="bg-[#3B82F6]/20 text-[#3B82F6] border-[#3B82F6]/30 px-4 py-2">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, i) => (
            <Card 
              ...
              className={`relative ${
                plan.highlighted 
                  ? 'bg-[#1E293B] border-[#3B82F6] border-2 shadow-lg shadow-[#3B82F6]/20' 
            ...
            >
              ...
              <CardHeader className="text-center pb-8 pt-10">
                ...
                <CardDescription className="text-[#94A3B8]">{plan.description}</CardDescription>
                <div className="mt-6">
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, j) => (
                    <li key={j} className="flex items-start gap-3">
                      <Check className="w-5 h-5 text-[#00FF41] flex-shrink-0 mt-0.5" />
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="grid grid-cols-4 gap-4 py-3 border-b border-[#334155] last:border-0">
                    <div className="font-semibold">{row.feature}</div>
                    <div className="text-sm text-[#94A3B8] text-center">{row.free}</div>
                    <div className="text-sm text-[#94A3B8] text-center">{row.pro}</div>
                    <div className="text-sm text-[#94A3B8] text-center">{row.enterprise}</div>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <DialogContent className="bg-[#1E293B] border-[#334155] max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between">
              ...
              <Badge className="bg-[#3B82F6]/20 text-[#3B82F6] border-none">
          ...
          <form onSubmit={handleCheckout} className="space-y-6 mt-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className="bg-[#0F172A] border-[#334155] font-mono pr-10"
                  required
                />
                <CreditCard className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[#94A3B8]" />
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-[#334155] space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-[#94A3B8]">{getSelectedPlanDetails()?.name}</span>
              ...
              <div className="flex justify-between text-lg font-bold pt-2 border-t border-[#334155]">
                <span>Total</span>
                <span className="font-mono text-[#00FF41]">${calculateTotal().toFixed(2)}</span>
              </div>
            </div>
